/* eslint-disable @typescript-eslint/no-unnecessary-condition, @typescript-eslint/no-misused-promises, @typescript-eslint/no-unused-vars -- vendor AI Elements code */
'use client';
...
const LineSpan = ({ keyedLine, showLineNumbers }: { keyedLine: KeyedLine; showLineNumbers: boolean }) => (
  <span className={showLineNumbers ? LINE_NUMBER_CLASSES : 'block'}>{keyedLine.tokens.length === 0 ? '\n' : keyedLine.tokens.map(({ token, key }) => <TokenSpan key={key} token={token} />)}</span>
);
...
const CodeBlockBody = memo(
  ({ tokenized, showLineNumbers, className }: { tokenized: TokenizedCode; showLineNumbers: boolean; className?: string }) => {
    ...
    return (
      <pre className={cn('dark:!bg-[var(--shiki-dark-bg)] dark:!text-[var(--shiki-dark)] m-0 p-4 text-sm', className)} style={preStyle}>
