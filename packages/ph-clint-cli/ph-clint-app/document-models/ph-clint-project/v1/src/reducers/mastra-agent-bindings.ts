...
import type { PhClintMainAgent, PhClintSubAgent } from '../../gen/schema/types.js';

type Agent = PhClintMainAgent | PhClintSubAgent;

function resolveAgent(main: PhClintMainAgent | null | undefined, subs: PhClintSubAgent[], agentId: string): Agent | undefined {
...
export const phClintProjectMastraAgentBindingsOperations: PhClintProjectMastraAgentBindingsOperations = {
  setAgentModelOperation(state, action) {
    ...
    const agent = resolveAgent(state.features.mastra.mainAgent, state.features.mastra.subAgents, action.input.agentId);
    if (!agent) {
