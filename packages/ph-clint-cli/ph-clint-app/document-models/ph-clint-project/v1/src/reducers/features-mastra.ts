...
import { InvalidAgentIdError, InvalidAgentNameError, MastraNotEnabledError, PowerhouseNotEnabledError } from '../../gen/features-mastra/error.js';
...
export const phClintProjectFeaturesMastraOperations: PhClintProjectFeaturesMastraOperations = {
  ...
  disableMastraOperation(state) {
    ...
    const ccIdx = state.packages.findIndex((p) => p.packageName === '@powerhousedao/clint-common' && p.managed);
    if (ccIdx !== -1) {
  ...
  setEnableChatOperation(state, action) {
    ...
    if (action.input.enabled) {
      ...
      const idx = state.packages.findIndex((p) => p.packageName === CLINT_COMMON_PKG && p.managed);
      if (idx !== -1) {
