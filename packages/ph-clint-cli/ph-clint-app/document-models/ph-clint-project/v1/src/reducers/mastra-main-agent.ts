...
import { InvalidAgentImageError, InvalidAgentNameError, MastraNotEnabledError } from '../../gen/mastra-main-agent/error.js';
...
export const phClintProjectMastraMainAgentOperations: PhClintProjectMastraMainAgentOperations = {
  ...
  setMainAgentImageOperation(state, action) {
    ...
    if (!action.input.attachment.startsWith('attachment://')) {
      throw new InvalidAgentImageError('Agent image must be an attachment ref (attachment://...)');
    }
