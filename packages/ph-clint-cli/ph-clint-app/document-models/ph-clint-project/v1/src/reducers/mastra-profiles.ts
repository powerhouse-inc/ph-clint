...
import { DuplicateProfileError, InvalidProfileIdError, MastraNotEnabledError, ProfileInUseError, ProfileNotFoundError } from '../../gen/mastra-profiles/error.js';
...
export const phClintProjectMastraProfilesOperations: PhClintProjectMastraProfilesOperations = {
  ...
  removeProfileOperation(state, action) {
    ...
    const usedBySub = state.features.mastra.subAgents.some((s) => s.profileIds.includes(action.input.id));
