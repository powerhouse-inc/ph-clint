...
import { DuplicateModelError, InvalidModelIdError, MastraNotEnabledError, ModelInUseError, ModelNotFoundError } from '../../gen/mastra-models/error.js';
...
export const phClintProjectMastraModelsOperations: PhClintProjectMastraModelsOperations = {
  addModelOperation(state, action) {
    ...
    state.features.mastra.models.push({ id: action.input.id });
  },
  removeModelOperation(state, action) {
    ...
    const usedBySub = state.features.mastra.subAgents.some((s) => s.modelId === action.input.id);
