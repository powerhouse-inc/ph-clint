...
describe("MastraSubAgentsOperations", () => {
  describe("ADD_SUB_AGENT", () => {
    ...
    it("trims name", () => {
      ...
      expect(doc.state.global.features.mastra.subAgents[0].name).toBe(
        "Summarizer",
      );
    });
  ...
  describe("SET_SUB_AGENT_DESCRIPTION", () => {
    it("updates the description", () => {
      ...
      doc = reducer(
        doc,
        setSubAgentDescription({ id: "sub", description: "new" }),
      );
      expect(doc.state.global.features.mastra.subAgents[0].description).toBe(
        "new",
      );
    });
    ...
    it("rejects when the sub-agent does not exist", () => {
      const doc = reducer(
        enabledWithExtraModel(),
        setSubAgentDescription({ id: "missing", description: "x" }),
      );
      const op = doc.operations.global[doc.operations.global.length - 1];
      expect(op.error).toContain("not found");
    });
