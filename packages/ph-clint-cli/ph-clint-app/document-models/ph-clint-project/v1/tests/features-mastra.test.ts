...
function createDocWithProfiles() {
  ...
  doc = reducer(
    doc,
    addProfile({ id: "style", title: "Style", content: "Style guide." }),
  );
  return doc;
}
...
describe("FeaturesMastraOperations", () => {
  describe("ENABLE_MASTRA", () => {
    ...
    it("should reject invalid agentId", () => {
      ...
      const updated = reducer(
        doc,
        enableMastra({ agentId: "Invalid-ID", agentName: "Agent" }),
      );
  ...
  describe("SET_DEFAULT_MODEL", () => {
    it("should switch default model", () => {
      ...
      expect(models.find((m) => m.id === "clint/demo-agent")!.isDefault).toBe(
        false,
      );
      expect(
      ...
      expect(
        models.find((m) => m.id === "anthropic/claude-haiku-4-5")!.isDefault,
      ).toBe(false);
    });
    ...
    it("should reject when mastra is disabled", () => {
      ...
      const updated = reducer(
        doc,
        setDefaultModel({ id: "anthropic/claude-sonnet-4-5" }),
      );
  ...
  describe("ADD_PROFILE", () => {
    ...
    it("should insert before specified profile", () => {
      ...
      doc = reducer(
        doc,
        addProfile({ id: "first", title: "First", content: "a" }),
      );
      doc = reducer(
        doc,
        addProfile({ id: "third", title: "Third", content: "c" }),
      );
      const updated = reducer(
        doc,
        addProfile({
    ...
    it("should reject invalid profile ID", () => {
      ...
      const updated = reducer(
        doc,
        addProfile({ id: "123bad", title: "T", content: "c" }),
      );
  ...
  describe("UPDATE_PROFILE", () => {
    ...
    it("should error on non-existent profile", () => {
      ...
      const updated = reducer(
        doc,
        updateProfile({ id: "nonexistent", title: "T" }),
      );
  ...
  describe("REORDER_PROFILES", () => {
    it("should move single profile to end", () => {
      ...
      const updated = reducer(
        doc,
        reorderProfiles({ ids: ["base"], insertBefore: null }),
      );
    ...
    it("should move single profile before another", () => {
      ...
      const updated = reducer(
        doc,
        reorderProfiles({ ids: ["style"], insertBefore: "base" }),
      );
    ...
    it("should move multiple profiles before another", () => {
      ...
      const updated = reducer(
        doc,
        reorderProfiles({ ids: ["tools", "style"], insertBefore: "base" }),
      );
    ...
    it("should move multiple profiles to end", () => {
      ...
      doc = reducer(
        doc,
        addProfile({ id: "extra", title: "Extra", content: "Extra." }),
      );
      const updated = reducer(
        doc,
        reorderProfiles({ ids: ["base", "tools"], insertBefore: null }),
      );
  ...
  describe("SET_AGENT_DESCRIPTION", () => {
    it("should set agent description when enabled", () => {
      ...
      const updated = reducer(
        doc,
        setAgentDescription({ description: "A helpful assistant" }),
      );

      expect(updated.state.global.features.mastra.agentDescription).toBe(
        "A helpful assistant",
      );
  ...
  it("should handle setAgentId operation", () => {
    const document = utils.createDocument();
    const input = generateMock(SetAgentIdInputSchema());

    const updatedDocument = reducer(document, setAgentId(input));
  ...
  it("should handle setDefaultModel operation", () => {
    const document = utils.createDocument();
    const input = generateMock(SetDefaultModelInputSchema());

    const updatedDocument = reducer(document, setDefaultModel(input));
  ...
  it("should handle clearAgentImage operation", () => {
    const document = utils.createDocument();
    const input = generateMock(ClearAgentImageInputSchema());

    const updatedDocument = reducer(document, clearAgentImage(input));
