...
function enabled(): PhClintProjectDocument {
  return reducer(utils.createDocument(), enableMastra({ agentId: 'test-agent', agentName: 'Test Agent' }));
}
...
describe('FeaturesMastraOperations', () => {
  describe('ENABLE_MASTRA', () => {
    it('seeds mainAgent + base model + base profile', () => {
      ...
      expect(s.profiles).toEqual([{ id: 'base', title: 'Base Profile', content: 'You are a helpful assistant.' }]);
    });

    it('trims agentName', () => {
      const doc = reducer(utils.createDocument(), enableMastra({ agentId: 'my-agent', agentName: '  My Agent  ' }));
      expect(doc.state.global.features.mastra.mainAgent!.name).toBe('My Agent');
    });

    it('rejects invalid agentId regex', () => {
      const doc = reducer(utils.createDocument(), enableMastra({ agentId: 'Bad-ID', agentName: 'X' }));
    ...
    it('rejects empty agentName', () => {
      const doc = reducer(utils.createDocument(), enableMastra({ agentId: 'my-agent', agentName: '   ' }));
  ...
  describe('DISABLE_MASTRA', () => {
    ...
    it('removes managed clint-common package when disabled', () => {
      ...
      expect(doc.state.global.packages.some((p) => p.packageName === '@powerhousedao/clint-common' && p.managed)).toBe(true);

      doc = reducer(doc, disableMastra({ _: true }));
      expect(doc.state.global.packages.some((p) => p.packageName === '@powerhousedao/clint-common')).toBe(false);
    });
  });
