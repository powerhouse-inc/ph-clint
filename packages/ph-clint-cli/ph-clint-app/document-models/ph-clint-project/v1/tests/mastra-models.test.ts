import { addModel, addSubAgent, enableMastra, reducer, removeModel, setAgentModel, utils, type PhClintProjectDocument } from 'document-models/ph-clint-project/v1';
...
describe('MastraModelsOperations', () => {
  describe('ADD_MODEL', () => {
    ...
    it('rejects when mastra is disabled', () => {
      const doc = reducer(utils.createDocument(), addModel({ id: 'anthropic/claude-sonnet-4-5' }));
  ...
  describe('REMOVE_MODEL', () => {
    ...
    it('removes an unused model', () => {
      let doc = enabled();
      doc = reducer(doc, addModel({ id: 'openai/gpt-4o' }));
      doc = reducer(doc, setAgentModel({ agentId: 'main', modelId: 'openai/gpt-4o' }));
