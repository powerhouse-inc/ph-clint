...
import { reducer, utils, isPhClintProjectDocument, setPowerhouseLevel, setPackageIdentifier } from 'document-models/ph-clint-project/v1';
...
describe('FeaturesPowerhouseOperations', () => {
  ...
  it('should auto-create managed app package with */* when enabling Powerhouse', () => {
    ...
    doc = reducer(doc, setPackageIdentifier({ identifier: '@myorg/my-tool-cli' }));
    const updated = reducer(doc, setPowerhouseLevel({ level: 'Reactor' as const }));

    const appPkg = updated.state.global.packages.find((p) => p.packageName === '@myorg/my-tool-app');
  ...
  it('should rename managed app package when identifier is set after enabling Powerhouse', () => {
    ...
    const updated = reducer(doc, setPackageIdentifier({ identifier: '@myorg/my-tool-cli' }));
