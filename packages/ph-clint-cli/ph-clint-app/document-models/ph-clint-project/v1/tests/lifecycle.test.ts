...
import { reducer, utils, importSpec, isPhClintProjectDocument } from 'document-models/ph-clint-project/v1';
...
describe('LifecycleOperations', () => {
  ...
  it('should dispatch importSpec with correct action type', () => {
    ...
    expect(updatedDocument.operations.global[0].action.type).toBe('IMPORT_SPEC');
  ...
  it('should import agentDescription and agentImage', () => {
    ...
    expect(updated.operations.global[0].error).toBeUndefined();
    expect(state.features.mastra.agentDescription).toBe('A helpful coding assistant');
    expect(state.features.mastra.agentImage).toBe('data:image/png;base64,iVBORw0KGgo=');
  });
  ...
  it('should import enableChat and add clint-common package', () => {
    ...
    const ccPkg = state.packages.find((p) => p.packageName === '@powerhousedao/clint-common');
  ...
  it('should not duplicate clint-common when already in packages', () => {
    ...
    expect(updated.operations.global[0].error).toBeUndefined();
    const ccPkgs = state.packages.filter((p) => p.packageName === '@powerhousedao/clint-common');
  ...
  it('should import deployment fields (proxyEnabled, supportedResources)', () => {
    ...
    expect(state.deployment.supportedResources).toEqual(['vetra-agent-s', 'vetra-agent-m']);
  });
  ...
  it('should validate agentImage as data URL on import', () => {
    ...
    expect(updated.operations.global[0].error).toBe('Invalid image: must be a data URL (data:<mime>;base64,...)');
