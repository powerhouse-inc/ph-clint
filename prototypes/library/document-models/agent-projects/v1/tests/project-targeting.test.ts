...
describe("ProjectTargetingOperations", () => {
  ...
  it("should handle runProject operation", () => {
    const document = utils.createDocument();
    const input = generateMock(RunProjectInputSchema());
  ...
  it("should handle stopProject operation", () => {
    const document = utils.createDocument();
    const input = generateMock(StopProjectInputSchema());
  ...
  it("should handle deleteProject operation", () => {
    const document = utils.createDocument();
    const input = generateMock(DeleteProjectInputSchema());
  ...
  describe("Smart logic", () => {
    ...
    it("should NOT auto-reconcile DELETED status for non-MISSING projects", async () => {
      ...
      const { updateProjectStatus } = await import(
        "@powerhousedao/agent-manager/document-models/agent-projects/v1"
      );
      document = reducer(
        document,
        updateProjectStatus({
    ...
    it("should clear runtime info when transitioning from RUNNING to STOPPED", async () => {
      ...
      const { updateRuntimeInfo, updateProjectStatus } = await import(
        "@powerhousedao/agent-manager/document-models/agent-projects/v1"
      );
