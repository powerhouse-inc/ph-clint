...
## ⛔ MANDATORY: Editors Are Created Through the MCP

**NEVER create editor boilerplate files manually.** The Vetra system generates the editor scaffold automatically when you register the editor through the MCP.

Your workflow is:

1. **`vetra-mcp__createDocument`** with type `powerhouse/document-editor` on the vetra drive (ID is auto-generated, NEVER set it manually)
2. **`vetra-mcp__addActions`** — configure with `ADD_DOCUMENT_TYPE`, `SET_EDITOR_NAME`, `SET_EDITOR_STATUS` (→ "CONFIRMED")
3. **Wait** for Vetra's code generator to produce the editor boilerplate in `editors/`
4. **Only then** implement the editor UI in the generated files

If you skip the MCP and write editor files directly, you will produce code that is missing generated hooks, incompatible with the reactor-browser runtime, and broken on the next codegen pass. **There are no exceptions to this rule.**
...
- If it's a new editor, create a new editor document on the "vetra-{hash}" drive via the MCP, of type `powerhouse/document-editor`
- Check the document editor schema with `vetra-mcp__getDocumentModelSchema` and comply with it
- After adding the editor document to the Vetra drive, a new editor will be generated in the `editors` folder — check `vetra-logs` to confirm
...
* **Check Prerequisites** [references/00.check-prerequisites.md](references/00.check-prerequisites.md)
* **Write Editor Implementation** [references/01.write-editor-implementation.md](references/01.write-editor-implementation.md)
