...
Make extensive use of the `vetra-mcp__*` tools for this scenario DM.02. 
...
- Ensure that the state schema is set in the document model spec using `vetra-mcp__*` tools, for the `global` scope
...
- Verify the document is correctly updated using `vetra-mcp__*` tools
...
### Update the document model specification using `vetra-mcp__*` tools
...
Now add the input types to the document model using `vetra-mcp__*` tools.
...
- Ensure that all modules, operations and their input types have been added to the document model spec via `vetra-mcp__*` tools, for the `global` scope
- Verify the document is correctly updated
