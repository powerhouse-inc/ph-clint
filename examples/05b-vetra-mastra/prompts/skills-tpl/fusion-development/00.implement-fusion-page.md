...
- Use `{{commands.reactor-packages-list.id}}` and `{{commands.vetra-start.id}}` to ensure the right Reactor Package backend project is running. Spin it up if needed.
- Use `{{commands.fusion-projects-list.id}}` and `{{commands.fusion-project-start.id}}` to ensure the correct Fusion project is running. Spin it up if needed.
