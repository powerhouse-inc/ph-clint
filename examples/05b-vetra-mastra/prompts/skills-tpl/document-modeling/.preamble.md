...
1. **Define** the model via `{{services.vetra.mcpPrefix}}createDocument` + `{{services.vetra.mcpPrefix}}addActions`
...
1. `{{services.vetra.mcpPrefix}}getDrives` — confirm which drives are available
2. `{{services.vetra.mcpPrefix}}getDocumentModelSchema` with `type: "powerhouse/document-model"` — learn the exact input schemas for `ADD_MODULE`, `ADD_OPERATION`, `SET_STATE_SCHEMA`, etc.
...
1. **`{{services.vetra.mcpPrefix}}createDocument`** — create the document model on the vetra drive (ID is auto-generated, NEVER set it manually)
2. **`{{services.vetra.mcpPrefix}}addActions`** — define the model: `SET_MODEL_ID`, `SET_STATE_SCHEMA`, `ADD_MODULE`, `ADD_OPERATION`, `ADD_OPERATION_ERROR`, etc.
3. **Check `{{commands.vetra-logs.id}}`** — confirm the code generator ran and produced files
