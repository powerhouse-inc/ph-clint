...
Make extensive use of the `{{services.vetra.mcpPrefix}}*` tools for this scenario DM.02. 
...
- Ensure that the state schema is set in the document model spec using `{{services.vetra.mcpPrefix}}*` tools, for the `global` scope
...
- Verify the document is correctly updated using `{{services.vetra.mcpPrefix}}*` tools
...
### Update the document model specification using `{{services.vetra.mcpPrefix}}*` tools
...
Now add the input types to the document model using `{{services.vetra.mcpPrefix}}*` tools.
...
- Ensure that all modules, operations and their input types have been added to the document model spec via `{{services.vetra.mcpPrefix}}*` tools, for the `global` scope
- Verify the document is correctly updated
