...
Make extensive use of the `{{services.reactor-project.mcpPrefix}}*` tools for this scenario DM.02. 
...
<!-- expert mode: present the state schema proposal and confirm with the user before submitting to MCP -->
...
- Ensure that the state schema is set in the document model spec using `{{services.reactor-project.mcpPrefix}}*` tools, for the `global` scope
...
- Verify the document is correctly updated using `{{services.reactor-project.mcpPrefix}}*` tools
...
### Update the document model specification using `{{services.reactor-project.mcpPrefix}}*` tools
...
Now add the input types to the document model using `{{services.reactor-project.mcpPrefix}}*` tools.
...
- Ensure that all modules, operations and their input types have been added to the document model spec via `{{services.reactor-project.mcpPrefix}}*` tools, for the `global` scope
- Verify the document is correctly updated
