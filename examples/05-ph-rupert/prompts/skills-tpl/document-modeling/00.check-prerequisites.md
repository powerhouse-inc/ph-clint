...
## DM.00.2 Use the reactor project management tools to run Vetra Studio and Vetra Switchboard
...
- Once the project is running, verify that the `{{services.reactor-project.mcpPrefix}}*` tools are available and responsive
- Request the Vetra Drive through the `{{services.reactor-project.mcpPrefix}}*` tools and verify you can see it.
- Verify that you see the accompanying Preview Drive too.

## DM.00.3 Review the existing package specs and implementation

Use the `{{services.reactor-project.mcpPrefix}}*` tools to complete this task.
...
- Request the Vetra Studio, Vetra Switchboard and MCP endpoints using `{{commands.reactor-project-ps.id}}`
...
- Make sure to share the Vetra Studio, Vetra Switchboard and MCP endpoints with the stakeholder for them to follow along.
