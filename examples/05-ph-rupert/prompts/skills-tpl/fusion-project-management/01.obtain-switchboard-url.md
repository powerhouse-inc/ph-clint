...
the Switchboard URL _before_ running the Fusion project through `{{commands.fusion-project-start.id}}`. If you fail to provide a
...
<!-- expert mode: confirm the backend choice with the user -->
<!-- discovery mode: explain what a Switchboard is and why the backend choice matters, then guide the user -->
<!-- one-shot mode: infer the best backend from available context and proceed -->
...
Use `{{commands.reactor-project-ls.id}}` and `{{commands.reactor-project-start.id}}` to ensure that the correct Reactor Package project is running and capture its endpoint.

Consult your capability scenario CRP.01 with the `self_reflection` MCP tool if needed for the details.
