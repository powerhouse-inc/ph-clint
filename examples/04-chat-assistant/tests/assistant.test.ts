...
describe('ascii command', () => {
  ...
  it('converts a remote image to ASCII art', async () => {
    const result = await ascii.execute(
      ...
      { workdir: '', workspace: createMemoryWorkdirStore(), stdout: () => {}, config: { model: 'test-model' }, runProcess: () => Promise.resolve({ success: true, output: '' }) },
    ) as any;
...
describe('save-image command', () => {
  ...
  it('downloads an image and saves to workspace', async () => {
    const result = await saveImage.execute(
      ...
      { workdir: testDir, workspace: createMemoryWorkdirStore(testDir), stdout: () => {}, config: { model: 'test-model' }, runProcess: () => Promise.resolve({ success: true, output: '' }) },
    ) as any;
...
describe('list-images command', () => {
  ...
  it('returns empty when no images exist', async () => {
    ...
    const result = await listImages.execute(
      {},
      { workdir: nonExistent, workspace: createMemoryWorkdirStore(nonExistent), stdout: () => {}, config: { model: 'test-model' }, runProcess: () => Promise.resolve({ success: true, output: '' }) },
    ) as any;
...
describe('REPL session with agent', () => {
  it('bare text routes to agent, /commands route directly', async () => {
    ...
    const context: CommandContext = { workdir: '', workspace: createMemoryWorkdirStore(), stdout: () => {}, config: {}, runProcess: () => Promise.resolve({ success: true, output: '' }) };
