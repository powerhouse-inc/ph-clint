...
1. **`{{services.vetra.mcpPrefix}}createDocument`** with type `powerhouse/document-editor` on the vetra drive (ID is auto-generated, NEVER set it manually)
2. **`{{services.vetra.mcpPrefix}}addActions`** — configure with `ADD_DOCUMENT_TYPE`, `SET_EDITOR_NAME`, `SET_EDITOR_STATUS` (→ "CONFIRMED")
...
- Check the document editor schema with `{{services.vetra.mcpPrefix}}getDocumentModelSchema` and comply with it
- After adding the editor document to the Vetra drive, a new editor will be generated in the `editors` folder — check `{{commands.vetra-logs.id}}` to confirm
...
Use `{{services.vetra.mcpPrefix}}addActions` with operations like:
