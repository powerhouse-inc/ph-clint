...
- Use the `{{commands.reactor-packages-list.id}}` tool to get all existing projects
...
- If a project is running, use `{{commands.vetra-stop.id}}` to stop it first
...
- Use `{{commands.reactor-package-init.id}}` with the generated project name
- Wait for the initialization to complete
- Capture the project path returned by the tool
- Use `{{commands.reactor-packages-list.id}}` to confirm the new project appears in the list and see its status