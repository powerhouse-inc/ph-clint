...
- Once the project is running, verify that the `{{services.vetra.mcpPrefix}}*` tools are available and responsive
- Request the Vetra drive through the `{{services.vetra.mcpPrefix}}*` tools and verify you can see it.
- Verify that you see the accompanying preview drive too.

## DM.00.3 Review the existing package specs and implementation

Use the `{{services.vetra.mcpPrefix}}*` tools to complete this task.
...
- Request the Vetra Connect, Switchboard and MCP endpoints using `{{commands.vetra-ps.id}}`
