...
- Use `{{commands.fusion-project-ps.id}}` with the project name
...
- Use `{{commands.fusion-project-stop.id}}` with the project name
- This will stop the Next.js service
- Wait for the shutdown command to complete
- Use `{{commands.fusion-project-ps.id}}` to confirm the project is now "stopped"
- Optionally get final logs with `{{commands.fusion-project-logs.id}}`